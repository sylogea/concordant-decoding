...
and $\Delta:\mathcal{P}(V)\times\mathcal{P}(V)\to[0,\infty]$ be the Kullback--Leibler (KL) divergence function.
...
\begin{equation}
	...
	\argmax_{y\in V}\left[
		\ell(y\mid x)
		+
		\Delta(\hat{xy}\ \Vert\ \hat{x})
	\right]
\end{equation}
where
$\hat{p}$ is the probability mass function satisfying
\[
	\hat{p}(v)
	=
	\Pr(v\mid p)
